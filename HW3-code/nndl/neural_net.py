...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    score_1 = np.maximum(0,np.dot(X,W1.T)+b1)
    scores = np.dot(score_1,W2.T)+b2
    ...
    #softmax_loss = np.sum(np.log(np.exp(scores[])) / np.sum(np.exp(scores)))
    softmax_loss = np.sum(np.log(np.sum(np.exp(scores),axis = 1))-scores[np.arange(N), y]) / N
    reg_loss = 0.5 * reg * (np.linalg.norm(W1, 'fro')**2 + np.linalg.norm(W2, 'fro')**2)
    #print(softmax_loss,reg_loss)
    ...
    nmzd = prob / N 
    weight_cvg = np.maximum(0, W1.dot(X.T) + b1.reshape([W1.shape[0], 1]))

    grads['W2'] = nmzd.T.dot(weight_cvg.T) + reg * W2

    grads['b2'] = np.sum(nmzd, axis=0, keepdims=True)

    con_1 = W2.T 
    weight_1 = (W1.dot(X.T) > 0) * con_1.dot(nmzd.T)
    grads['W1'] = weight_1.dot(X) + reg * W1

    grads['b1'] = np.sum(weight_1, axis=1, keepdims=True).T
  ...
  def predict(self, X):
    ...
    # ================================================================ #
    score_1 = np.dot(X, self.params['W1'].T) + self.params['b1']
    scores = np.dot(np.maximum(0, score_1), self.params['W2'].T) + self.params['b2']
