...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    out_conv_height = (H + 2 * padding - filter_size) / stride + 1
    out_conv_width = (W + 2 * padding - filter_size) / stride + 1

    self.params['W1'] = np.random.normal(0, weight_scale, [num_filters, C, filter_size, filter_size])
    self.params['b1'] = np.zeros([num_filters]) 
    
   
    pool_height = int((out_conv_height - 2) / 2 + 1)
    pool_width = int((out_conv_width - 2) / 2 + 1)
  ...
  def loss(self, X, y=None):
    ...
    reg_loss = self.reg * 0.5 * (np.sum(np.square(W1)) + np.sum(np.square(W2)) + np.sum(np.square(W3)))
