  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      ...
      if i == self.num_layers -1:
        ...
        out_f, cache_f = affine_forward(x_l , self.params['W'+str(i+1)], self.params['b'+str(i+1)])

      else:
        if self.use_batchnorm:
          
          out, cache = affine_batchNorm_relu_forward(x_l, self.params['W'+str(i+1)], self.params['b'+str(i+1)], self.params['gamma' + str(i+1)], self.params['beta'+str(i+1)], self.bn_params[i])
        else:
          out, cache = affine_relu_forward(x_l,self.params['W'+str(i+1)], self.params['b'+str(i+1)])
        
        if self.use_dropout : 
          out, drop_fwd = dropout_forward(out, self.dropout_param)

          cache = (cache, drop_fwd)

        x_l = out
        total_l.append(cache)
    ...
    loss,d_s = softmax_loss(out_f, y)

    for i in range(self.num_layers):

      regu_loss = 0.5 * self.reg * np.sum(self.params['W'+str(i+1)] * self.params['W'+str(i+1)]) 
      loss += regu_loss

    for i in reversed(range(self.num_layers)):

      if i == self.num_layers - 1:
        dx1, grads['W'+str(i+1)], grads['b'+str(i+1)] = affine_backward(d_s,cache_f)
      else:
        cache = total_l[i]

        if self.use_dropout:

          cache, drop_fwd = cache
          dx1 = dropout_backward(dx1, drop_fwd)
