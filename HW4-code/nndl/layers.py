...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_j = (x - batch_mean) / np.sqrt(batch_var + eps)
        out = gamma * x_j + beta
        ...
        cache = {}
        cache['normalize'] = x_j
        cache['variance'] = batch_var
        cache['center'] = x-batch_mean
        ...
        # ================================================================ #
        x_v = (x - running_mean) / np.sqrt(running_var + eps)
        out = gamma * x_v  + beta
...
def batchnorm_backward(dout, cache):
    ...
    dgamma = np.sum(np.multiply(dout, cache['normalize']), axis = 0)

    batch_var = cache['variance']
    center_x = cache['center']
    x_normalize = cache['normalize']
    gamma = cache['gamma']
    eps = cache['eps']
    x = cache['x']
    mean_v = cache['center']
    ...
    den = 1/(batch_var + eps)
    dx = gamma * (np.sqrt(den)/x.shape[0])  * (x.shape[0] * dout  - (dout.sum(axis = 0)) - ((mean_v * den)) * ((mean_v * dout).sum(axis = 0)))
