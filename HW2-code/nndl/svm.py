...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    # ================================================================ #
      ...
      diff = temp + 1 - temp[temp_y]
      diff[temp_y] = 0
      diff = np.maximum(diff, 0)
      temp_loss = np.sum(diff)
      loss += temp_loss
      #print(loss)
    loss /= num_train  
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
      temp_y = y[i]
      temp_score = np.dot(self.W, X[i])
      
      diff = temp_score + 1 - temp_score[temp_y]
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    index_pred = y, range(len(y)) 
    pred_score = total_score[index_pred]

    diff = total_score - pred_score + np.ones(total_score.shape)
    diff[index_pred] = 0
    ...
    # ================================================================ #
    diff[diff > 0] = 1
    diff[index_pred] = -1 * np.sum(diff, axis=0)
    #print(diff)

    grad = np.dot(diff, X) / X.shape[0]
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
